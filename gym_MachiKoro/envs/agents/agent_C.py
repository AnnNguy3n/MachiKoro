...
class Agent(Player):
    ...
    def action(self, dict_input):
        ...
        for card in dict_input["Board"].support_cards.keys():
            if card not in dict_input['Cards_bought'] and max(dict_input["Board"].support_cards_object[card].value_to_activate) < 4 and dict_input["Board"].support_cards_object[card].price <= self.coins and dict_input["Board"].support_cards[card] > 0 and card != "Cafe" and card != 'Business Complex':
                return card